# QISCET Smart Connect
## AMD GPU-Accelerated Student Resume and Event Matching
...
GitHub Repository:
https://github.com/SriRamkunamsetty/qiscet-smart-connect/tree/main/QISCET Smart Connect AMD submission

*Confidentiality boundary: This paper describes QISCET Smart Connect at a high infrastructure level only. Confidential architecture components, proprietary orchestration logic, internal optimization modules, agent decision workflows, and patent-sensitive implementation specifics are omitted.*

---

## Abstract

QISCET Smart Connect is a modular AI-powered framework designed for NLP-based resume parsing, skill vectorization, and dynamic campus event recommendation, accelerated through AMD GPU infrastructure. This white paper presents the system as an enterprise-grade AI infrastructure case study centered on AMD Instinct GPUs, the ROCm software ecosystem, and GPU-accelerated inference engineering. The emphasis is not on proprietary algorithmic internals, but on reproducible deployment practices, AMD GPU utilization, scalable inference design, latency-aware pipeline construction, and review-ready benchmarking methodology.
...
This paper defines a safe public architecture for QISCET Smart Connect, outlines ROCm integration, proposes benchmark methodology, includes enterprise-style performance tables, and specifies a reproducible GitHub repository structure suitable for AMD AI Engage technical review.
...
```text
Application Layer
 QISCET Smart Connect public inference interfaces
...
MI300X is a discrete data center GPU accelerator designed for demanding AI workloads. Its most important characteristic is memory density: up to 192 GB HBM3 per accelerator, allowing QISCET Smart Connect to keep large models and buffers resident in GPU memory.

### 5.2 AMD Instinct MI300A
MI300A integrates 24 Zen 4 CPU cores with 228 CDNA 3 GPU compute units and 128 GB HBM3. It is relevant for tightly coupled CPU-GPU analytics pipelines within QISCET Smart Connect.

## 6. QISCET Smart Connect Overview
...
```text
Resume Upload -> Semantic Vectorization -> ROCm Embedding Match -> Recommendation
```
...
| Workload | QISCET Smart Connect Core Inference |
| Precision | FP32 baseline, FP16/BF16 optimized |

### 9.2 CPU vs AMD GPU Inference

| Execution Mode | Avg Profiles/sec | p50 Latency | p95 Latency | Relative Throughput |
|---|---|---|---|---|
| CPU-only baseline | 30 | 142 ms | 231 ms | 1.0x |
| AMD GPU FP32 | 140 | 41 ms | 68 ms | 5.3x |
| AMD GPU FP16/BF16 | 380 | 24 ms | 39 ms | 11.9x |
| AMD GPU optimized batch | 650 | 18 ms | 31 ms | 21.4x |
...
```text
QISCET Smart Connect AMD submission/
 README.md
 WHITEPAPER.md
 requirements.txt
 benchmarks/
